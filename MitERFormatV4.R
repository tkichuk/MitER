##NOTE: This script is set up to be run in a folder containing two excel files. 
#SummaryFile.xlsx should contain morphological data and volume measurements
#TiterSummary.xlsx should contain all titer measurements
...
## Save R script to the folder on your desktop that contains your summary excel file
DataFolder <- dirname(rstudioapi::getSourceEditorContext()$path)
SummaryPath <- file.path(DataFolder, "Data")
setwd(SummaryPath)
getwd()
...
for(i in CellStatsFiles){
  ...
  if (nrow (DF) == 34){
    ...
    print (CellID)
    DFinal <- cbind (File, Condition, CellID, DFinal)
...
for(i in DistributionFiles){
  ...
  if (nrow (DF) > 0){  # Make sure file is not empty
    # ~ Formatting ~
    RowOne <- colnames(DF2)
    ColTwo <- rbind (RowOne,DF2[1])
    DF2 <- as.data.frame (ColTwo[,1])
    DFinal2 <- data.frame(matrix(DF2[,1], ncol = 4, byrow = TRUE))
    ColNames <- c ("SectionName", "CubeSA", "DifferenceSA", "SectionVerts")
    ...
    DFinaler <- merge (DFinal, CubeNames, by= "SectionVerts") 
      ...
      FaceSum <- sum(as.numeric(FaceList$PercentVol))
      CellFaceSum <- sum(as.numeric(FaceList$CellPercentVol))
      NormFaceSum <- FaceSum/CellFaceSum
      MitoFaceSumSA <- sum(as.numeric(FaceList$PercentSectionMitoSA))
      
      
      EdgeList <- DFinaler[DFinaler$Designation == "Edge",]
      EdgeSum <- sum(as.numeric(EdgeList$PercentVol))
      CellEdgeSum <- sum(as.numeric(EdgeList$CellPercentVol))
      NormEdgeSum <- EdgeSum/CellEdgeSum
      MitoEdgeSumSA <- sum(as.numeric(EdgeList$PercentSectionMitoSA))
      
      CornerList <- DFinaler[DFinaler$Designation == "Corner",]
      CornerSum <- sum(as.numeric(CornerList$PercentVol))
      CellCornerSum <- sum(as.numeric(CornerList$CellPercentVol))
      NormCornerSum <- CornerSum/CellCornerSum
      MitoCornerSumSA <- sum(as.numeric(CornerList$PercentSectionMitoSA))
      
      CenterList <- DFinaler[DFinaler$Designation == "Center",]
      CenterSum <- sum(as.numeric(CenterList$PercentVol))
      CellCenterSum <- sum(as.numeric(CenterList$CellPercentVol))
      NormCenterSum <- CenterSum/CellCenterSum
      MitoCenterSumSA <- sum(as.numeric(CenterList$PercentSectionMitoSA))
  ...
  NormContactPeriphPercent <- ContactPeriphPercent/MitoPeriphPercentSA
  NormContactCenterPercent <- ContactCenterPercent/MitoCenterPercentSA
  Metric <- as.data.frame (t(c(File, ContactPeriphSum, ContactCenterSum, TotalContactSum, ContactPeriphPercent, ContactCenterPercent, NormContactPeriphPercent, NormContactCenterPercent)))
  colnames(Metric) <-c("File", "ContactPeriphSum", "ContactCenterSum", "TotalContactSum", "ContactPeriphPercent", "ContactCenterPercent", "NormContactPeriphPercent", "NormContactCenterPercent")
...
SummaryFile <- merge(CellStats,CutoffOverlapStats[-1,],by="File")
...
# Merging all stats into one dataframe
SummaryFile <- merge(SummaryFile,FullMitoStats[-1,],by="File")

# Replacing experimental conditions with descriptive names
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "WTGlutoGlu", replacement = "Fermentation", x = t))
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "WTGEtoGE", replacement = "Respiration", x = t))
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "WTGlutoGE", replacement = "Transition", x = t))
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "TK7GlutoGlu", replacement = "FermentationKO", x = t))
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "TK7GEtoGE", replacement = "RespirationKO", x = t))
SummaryFile$Condition <-lapply(X = SummaryFile$Condition, FUN = function(t) gsub(pattern = "TK7GlutoGE", replacement = "TransitionKO", x = t))

# Changing everything that's number to be numeric
SummaryFile[,!colnames(SummaryFile) %in% c("File", "CellID", "Condition", "CenterOfGravity.Mitochondria", "BBVerts.Mitochondria", "CenterOfGravity.MitoNodes", "BBVerts.MitoNodes", "CenterOfGravity.ER", "BBVerts.ER", "CenterOfGravity.ERNodes", "BBVerts.ERNodes", "CenterOfGravity.Cell", "BBVerts.Cell")] <- mutate_all(SummaryFile[,!colnames(SummaryFile) %in% c("File", "CellID", "Condition", "CenterOfGravity.Mitochondria", "BBVerts.Mitochondria", "CenterOfGravity.MitoNodes", "BBVerts.MitoNodes", "CenterOfGravity.ER", "BBVerts.ER", "CenterOfGravity.ERNodes", "BBVerts.ERNodes", "CenterOfGravity.Cell", "BBVerts.Cell")], function(x) as.numeric(as.character(x)))
ColumnNames <- colnames(SummaryFile)
SummaryFile <- data.frame(matrix(unlist(SummaryFile), nrow=nrow(SummaryFile)),stringsAsFactors=FALSE)
colnames(SummaryFile) <- ColumnNames

# Drop rows with NA values
SimpleSummary <- SummaryFile[complete.cases(SummaryFile), ]

# Changing everything that's number to be numeric
SimpleSummary[,!colnames(SummaryFile) %in% c("File", "CellID", "Condition", "CenterOfGravity.Mitochondria", "BBVerts.Mitochondria", "CenterOfGravity.MitoNodes", "BBVerts.MitoNodes", "CenterOfGravity.ER", "BBVerts.ER", "CenterOfGravity.ERNodes", "BBVerts.ERNodes", "CenterOfGravity.Cell", "BBVerts.Cell")] <- mutate_all(SimpleSummary[,!colnames(SimpleSummary) %in% c("File", "CellID", "Condition", "CenterOfGravity.Mitochondria", "BBVerts.Mitochondria", "CenterOfGravity.MitoNodes", "BBVerts.MitoNodes", "CenterOfGravity.ER", "BBVerts.ER", "CenterOfGravity.ERNodes", "BBVerts.ERNodes", "CenterOfGravity.Cell", "BBVerts.Cell")], function(x) as.numeric(as.character(x)))
...
while (i <= Counter) {
  ...
  Names <- colnames(DF)
  ColumnNames <- char2end(Names, ".")
  colnames(DF) <- c(ColumnNames)
  drops <- c("File", "CellID", "Condition", "CenterOfGravity.Mitochondria", "BBVerts.Mitochondria", "CenterOfGravity.MitoNodes", "BBVerts.MitoNodes", "CenterOfGravity.ER", "BBVerts.ER", "CenterOfGravity.ERNodes", "BBVerts.ERNodes", "CenterOfGravity.Cell", "BBVerts.Cell")
...
colnames (OutlierFree)
colnames (OutlierFree) <- colnames(SimpleSummary)

SimpleSummary$Condition <- factor(SimpleSummary$Condition, levels = unique(SimpleSummary$Condition))
...
OutlierFree$MeanCellRadius <-((OutlierFree$BBHeight.Cell+OutlierFree$BBLength.Cell+ OutlierFree$BBWidth.Cell)/3)/2
OutlierFree$AsymmetryMetric <- (OutlierFree$COGDifference.Mitochondria/ OutlierFree$MeanCellRadius)

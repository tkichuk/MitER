...
def ThreeStructureLabelMaker (mesh_obs, SortedBBList, other_out):
    ...
    for o in mesh_obs:
        ...
        if round(BBVolume, 2) == round(BBCell, 2) and volume == VolCell:
            pass

        elif round(BBVolume, 2) == round(BBCell, 2) and volume < VolCell:
            o = FirstBoolForLabelMaker (o, volume)
          
        elif round(BBVolume, 2) != round(BBCell, 2):
            o, other_out = SecondBoolForLabelMaker (o, other_out, volume)
...
def TwoStructureLabelMaker (mesh_obs, SortedBBList, other_out):
    ...
    mesh_ob = bpy.context.scene.objects["Mitochondria"]
    ob.select_set(True)
    #ob.name = 'Copy'
    bpy.ops.object.select_all(action='DESELECT')
    objectToSelect = bpy.data.objects[mesh_ob.name]
        ...
        for file in OnlyFullFiles:
                ...
                if len(mesh_obs) == 5:
                    ...
                    for fraction in fraction_list:
                        ...
                        bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY')
                        newer_obj.scale *= math.pow(n, 1/3)

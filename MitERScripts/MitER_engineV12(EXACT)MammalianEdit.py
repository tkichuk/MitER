...
def GetRawVolumeAndDimensions (mesh_object_list, BBList, vol_list): 
    ...
    for o in mesh_object_list:
        ...
        Input  = vol_list
        RawOutput = sorted (Input, key = lambda x:float(x))
...
def GetVolumeAndDimensions (mesh_object_list, BBList, vol_list): 
    ...
    for o in mesh_object_list:
        ...
        remesh_modifier.voxel_size = RemeshFactor
...
def IdentifyAndNameCell (mesh_obs, rawoutput):
    ...
    for o in mesh_obs:
        ...
        if volume == rawoutput [-1]:
            ...
            remesh_modifier = new_obj.modifiers.new(name='Remesh', type='REMESH')
            remesh_modifier.use_remove_disconnected = True
            remesh_modifier.use_smooth_shade = True
            remesh_modifier.mode = 'VOXEL'
            remesh_modifier.voxel_size = CellRemeshFactor
            ...
            new_obj.scale *= CellScale
            new_obj.name = "UpscaledCell"
            
            ob = bpy.context.scene.objects ["UpscaledCell"]    
            bpy.ops.object.select_all (action='DESELECT')
            bpy.context.view_layer.objects.active = ob  
            bpy.ops.object.editmode_toggle()
            bpy.ops.mesh.print3d_clean_non_manifold()
...
def FourStructureLabelMaker (mesh_obs, SortedBBList, other_out, rawoutput):
    ...
    for o in mesh_obs:
        ...
        elif volume == rawoutput [2] and o.name != 'UpscaledCell':
            o.name = 'ER'
        
        elif volume == rawoutput [1] and o.name != 'UpscaledCell':
            o.name = 'ER'
            
        elif volume == rawoutput [0]:
            o, other_out = SecondBoolForLabelMaker (o, other_out, volume)
...
def ThreeStructureLabelMaker (mesh_obs, SortedBBList, other_out, output):
    ...
    for o in mesh_obs:
        ...
        elif volume == output [2] and o.name != 'UpscaledCell':
            o.name = 'ER'
        
        elif volume == output [1] and o.name != 'UpscaledCell':
            o.name = 'ER'
          
        elif volume == output [0]:
            o, other_out = SecondBoolForLabelMaker (o, other_out, volume)
...
def CSV_Writer (Desktop, SmallFileName, out_list, OtherOut, DistData, CellDistData, ContactDistData, OverlapList ):
    ...
    file = open(Desktop+'/MitER/Data/'+SmallFileName +'-MitochondrialDistribution.csv', 'w', newline='')
    with file:
        write = csv.writer(file)
    ...
    file = open(Desktop+'/MitER/Data/'+SmallFileName +'-MitochondrialAreaDistribution.csv', 'w', newline='')
    with file:
        write = csv.writer(file)
...
class OT_TestOpenFilebrowser (Operator, ImportHelper):
    ...
    def execute(self, bpycontext):
        ...
        OnlyFullFiles = [f for f in OnlyFiles if FileExtension in f]
        ...
        for file in OnlyFullFiles:
                ...
                # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                RawOutput = GetRawVolumeAndDimensions (mesh_obs, BBList, vol_list)
                mesh_obs = IdentifyAndNameCell (mesh_obs, RawOutput)
                ...
                #This will run if there are only three structures (one of them must be the cell)
                    ...
                    mesh_obs = ThreeStructureLabelMaker ( mesh_obs, SortedBBList, OtherOut, Output ) 
                ...
                #This will run if there are only four structures (one of them must be the cell)
                elif len(mesh_obs) == 4:
                    #mesh_obs = IdentifyAndNameCell (mesh_obs, Output)

                    mesh_obs = FourStructureLabelMaker ( mesh_obs, SortedBBList, OtherOut, Output ) 

...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# // NAME OF FUNCTION: chop
# //
# // PARAMETERS: ob, start, end, segments, size, m, chopper, mw, bpycontext
# //
# // WHAT THE FUNCTION DOES: This function subdivides an object (in this case a cube)  
# // by using a difference boolean
# // 
# //
# // DEPENDENCIES: None
# //
...
def chop(ob, start, end, segments, size, m, chopper, mw, bpycontext):
    slices = []
    ...
    for i, p in planes:
        ...
        bpycontext.scene.collection.objects.link(cp)
...
def update_names_t1 (collated_cubes, mitochondria_object, b_objects, dist_data):
    ...
    for o in collated_cubes:
        ...
        bewl.operation = "INTERSECT" 
        bewl.solver = "FAST"
        bpy.ops.object.modifier_apply ({"object": Cube}, modifier=bewl.name)

        # create triangularted mesh
...
def triangulate_object_t1(obj, dist_data, plain_verts):
    ...
    bmesh.ops.triangulate(bm, faces=bm.faces[:])
    volume = bm.calc_volume()
    area = {obj:sum([f.area for f in obj.data.polygons])}
...
def update_names_t2 (collated_cubes, mitochondria_object, b_objects, contact_dist_data):
    ...
    for o in collated_cubes:
        ...
        print (o.name)
        contact_dist_data.append (o.name)

        
        # get volume of intersection between cube and mitochondria 
        bewl.operation = "DIFFERENCE" 
        bewl.solver = "FAST"
        bpy.ops.object.modifier_apply ({"object": Cube}, modifier=bewl.name)
...
def update_names_t3 (collated_cubes, cell_object, b_objects, cell_dist_data):
    ...
    for o in collated_cubes:
        ...
        bbverts     = [o.matrix_world @ Vector(bbvert) for bbvert in o.bound_box]
        plain_verts = [vert.to_tuple() for vert in bbverts] 
        bewl        = Cube.modifiers.new(type="BOOLEAN", name="CellDist")
        bewl.object = cell_object 
...
class OT_TestOpenFilebrowser (Operator, ImportHelper):
    ...
    def execute(self, bpycontext):
        ...
        OnlyFullFiles = [f for f in OnlyFiles if '.x3d' in f]
        ...
        for file in OnlyFullFiles:
                ...
                if len(mesh_obs) == 5:
                    ...
                    bpy.ops.mesh.primitive_cube_add(size=2, enter_editmode=False,  location=(global_bbox_center), scale=(bpy.data.objects['Cell'].dimensions), rotation=(bpy.data.objects['Cell'].rotation_euler))
                    ...
                    bpy.ops.mesh.primitive_plane_add(location=mw @ bbox_center(ob), size=size)
                    ...
                    for ox in chop(ob, o, x, segments_x, size, m, chopper, mw, bpycontext):
                        for oy in chop(ox, o, y, segments_y, size, m, chopper, mw, bpycontext):
                            chop(oy, o, z, segments_z, size, m, chopper, mw, bpycontext)
                    ...
                    # this almost seems like an entire chunk of independent code?
                ...
                #This will run if there are only two structures (one of them must be the cell)
                    ...
                    # Extract out the objects labelled Cell
                    objects = bpy.data.objects   
                    Cell = objects['Cell']
                    bm = bmesh.new()
